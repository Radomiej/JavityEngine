...
public class NameGenerator {
	...
	public void refresh() throws IOException {
		...
		try {
			reader = new InputStreamReader(classLoader.getResourceAsStream(fileName));
		} catch (Exception e) {
			reader = new InputStreamReader(classLoader.getResourceAsStream("resources/" + fileName));
		}
	...
	public String compose(int syls) {
		...
		String name;
		int a = (int) (JRandom.INSTANCE.nextDouble() * pre.size());
		...
		for (int i = 0; i < b.length - 2; i++) {
			...
			do {
				b[i] = (int) (JRandom.INSTANCE.nextDouble() * mid.size());
				// System.out.println("exp " +expecting+"
				// vocalF:"+vocalFirst(mid.get(b[i]))+" syl: "+mid.get(b[i]));
			} while (expecting == 1 && vocalFirst(pureSyl(mid.get(b[i]))) == false
		...
		int c;
		do {
			c = (int) (JRandom.INSTANCE.nextDouble() * sur.size());
