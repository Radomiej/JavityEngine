...
//		dependencies.add(dependencyBank.getDependency(ProjectDependency.JAVITY));
...
//		dependencies.add(dependencyBank.getDependency(ProjectDependency.MESSAGE_BUS));
	...
	void generate() {
		...
		new Thread() {
			public void run() {
				...
				new GdxSetup().build(builder, destination, name, gameIdentifier, pack, clazz, sdkLocation,
						new CharCallback() {
					@Override
	...
	class UI extends JPanel {
		...
		JLabel windowLabel = new JLabel("    Javity Project Generator");
		JButton exit;
		JButton minimize;
		JLabel logo;

		{
			...
			form.setBackground(new Color(239, 239, 239));
			for (int i = 0; i < form.getComponents().length; i++) {
				Component component = form.getComponents()[i];
				if (component instanceof JTextField) {
					component.setBackground(new Color(254, 254, 254));
					component.setForeground(new Color(46, 46, 46));
			...
			textArea.setForeground(new Color(46, 46, 46));

			Border line = BorderFactory.createLineBorder(Color.DARK_GRAY);
			scrollPane.setBorder(line);

			try {
				BufferedImage exitimg = ImageIO
						.read(GdxSetupUI.class.getResourceAsStream("/com/badlogic/gdx/setup/data/exitup.png"));
				...
				BufferedImage img = ImageIO
						.read(GdxSetupUI.class.getResourceAsStream("/com/badlogic/gdx/setup/data/logo.png"));
		...
		private void uiLayout() {
			...
			topBar.add(windowLabel,
					new GridBagConstraints(0, 0, 0, 0, 0, 0, NORTHWEST, VERTICAL, new Insets(0, 0, 0, 0), 0, 0));
	...
	class Form extends JPanel {
		...
		JTextField nameText = new JTextField("my-javity-game");
		...
		JTextField gameClassText = new JTextField("MyJavityGameScene");
		...
		JLabel versionLabel = new JLabel("Javity Version");
...

		{
			...
			for (int i = 0; i < releaseItems.length; i++) {
				releaseItems[i] = Releases.getReleases()[i].getJavityVersion();
			}
			versionButton = new JComboBox(releaseItems);
		}

		JLabel projectsLabel = new JLabel("Sub Projects");
		JLabel extensionsLabel = new JLabel("Javity / LibGDX Extensions");
		...
		private void uiStyle() {
			nameText.setCaretColor(Color.GRAY);
			gameIdentifierText.setCaretColor(Color.GRAY);
			packageText.setCaretColor(Color.GRAY);
			gameClassText.setCaretColor(Color.GRAY);
			destinationText.setCaretColor(Color.GRAY);
			sdkLocationText.setCaretColor(Color.GRAY);

			nameLabel.setForeground(new Color(46, 46, 46));
			gameIdentifierLabel.setForeground(new Color(46, 46, 46));
			packageLabel.setForeground(new Color(46, 46, 46));
			gameClassLabel.setForeground(new Color(46, 46, 46));
			destinationLabel.setForeground(new Color(46, 46, 46));
			sdkLocationLabel.setForeground(new Color(46, 46, 46));
			sdkLocationText.setDisabledTextColor(Color.GRAY);

			versionLabel.setForeground(new Color(46, 46, 46));
			UIManager.put("ComboBox.selectionBackground", new ColorUIResource(new Color(169, 169, 169)));
			UIManager.put("ComboBox.selectionForeground", new ColorUIResource(new Color(46, 46, 46)));
			versionButton.updateUI();
			versionButton.setBorder(BorderFactory.createLineBorder(new Color(46, 46, 46)));
			versionButton.setForeground(new Color(46, 46, 46));
			versionButton.setBackground(new Color(239, 239, 239));
			versionButton.setPrototypeDisplayValue("I am a prototype");
			versionButton.setUI(new BasicComboBoxUI() {
				@Override
				protected JButton createArrowButton() {
					return new BasicArrowButton(BasicArrowButton.SOUTH, new Color(239, 239, 239), new Color(0, 0, 0),
							new Color(100, 100, 100), new Color(100, 100, 100));
				}
			});

			projectsLabel.setForeground(new Color(46, 46, 46));
			extensionsLabel.setForeground(new Color(46, 46, 46));

			subProjectsPanel.setOpaque(true);
			subProjectsPanel.setBackground(new Color(239, 239, 239));

			for (JPanel extensionPanel : extensionsPanels) {
				extensionPanel.setOpaque(true);
				extensionPanel.setBackground(new Color(239, 239, 239));
			}
		}
		...
		private void uiLayout() {
			...
			add(gameIdentifierLabel,
					new GridBagConstraints(0, 1, 1, 1, 0, 0, EAST, NONE, new Insets(0, 0, 6, 6), 0, 0));
			add(gameIdentifierText,
					new GridBagConstraints(1, 1, 2, 1, 1, 0, CENTER, HORIZONTAL, new Insets(0, 0, 6, 0), 0, 0));

			add(packageLabel, new GridBagConstraints(0, 2, 1, 1, 0, 0, EAST, NONE, new Insets(0, 0, 6, 6), 0, 0));
			add(packageText,
					new GridBagConstraints(1, 2, 2, 1, 1, 0, CENTER, HORIZONTAL, new Insets(0, 0, 6, 0), 0, 0));

			add(gameClassLabel, new GridBagConstraints(0, 3, 1, 1, 0, 0, EAST, NONE, new Insets(0, 0, 6, 6), 0, 0));
			add(gameClassText,
					new GridBagConstraints(1, 3, 2, 1, 1, 0, CENTER, HORIZONTAL, new Insets(0, 0, 6, 0), 0, 0));

			add(destinationLabel, new GridBagConstraints(0, 4, 1, 1, 0, 0, EAST, NONE, new Insets(0, 0, 0, 6), 0, 0));
			add(destinationText,
					new GridBagConstraints(1, 4, 1, 1, 1, 0, CENTER, HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
			...
			add(sdkLocationText,
					new GridBagConstraints(1, 5, 1, 1, 1, 0, CENTER, HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
			add(sdkLocationButton,
					new GridBagConstraints(2, 5, 1, 1, 0, 0, CENTER, NONE, new Insets(0, 6, 0, 0), 0, 0));
			...
			add(subProjectsPanel,
					new GridBagConstraints(0, 8, 3, 1, 0, 0, CENTER, HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
...
//							depCheckBox.setSelected(true);
//						} else if (projDep.equals(ProjectDependency.MESSAGE_BUS)) {
//							depCheckBox.setSelected(true);
//						}
			...
			for (JPanel extensionsPanel : extensionsPanels) {
				add(extensionsPanel, new GridBagConstraints(0, rowCounter, 3, 1, 0, 0, CENTER, HORIZONTAL,
						new Insets(5, 0, 0, 0), 0, 0));
				rowCounter++;
			}
//			add(showMoreExtensionsButton,
//					new GridBagConstraints(0, rowCounter, 0, 1, 0, 0, CENTER, WEST, new Insets(20, 0, 0, 0), 0, 0));
		}
	...
	public static class SetupCheckBox extends JCheckBox {
		...
		private static BufferedImage getCheckboxImage(String imageName) throws IOException {
			return ImageIO
					.read(GdxSetupUI.class.getResourceAsStream("/com/badlogic/gdx/setup/data/" + imageName + ".png"));
		}
		...
		public Icon getPressedIcon() {
			// checkbox is missing 'pressed selected' icon, this allows us to
			// add it
			if (isSelected())
	...
	public static class SetupButton extends JButton {
		private Color textColor = new Color(46, 46, 46);
		...
		SetupButton(String buttonTag) {
			...
			Border line = BorderFactory.createLineBorder(new Color(46, 46, 46));
