...
public class GdxSetupUI extends JFrame {
	...
	public GdxSetupUI () {
		setTitle("Javity Project Generator");
		...
		dependencies.add(builder.bank.getDependency(ProjectDependency.JAVITY));
	}
	...
	class Form extends JPanel {
		...
		JTextField packageText = new JTextField("com.myjavity.game");
		JLabel gameClassLabel = new JLabel("Game class:");
		JTextField gameClassText = new JTextField("MyJavityGame");
			...
			for (int row = 0; row <= (ProjectDependency.values().length / 5); row++) {
				...
				while (depCounter < ProjectDependency.values().length) {
					if (ProjectDependency.values()[depCounter] != null) {
						...
						if (projDep.equals(ProjectDependency.JAVITY)) {
